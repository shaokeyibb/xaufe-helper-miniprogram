...
Component({
     ...
     */
    methods: {
        getDataUri: function getDataUri(name, color) {
            var that = this;
            var url = (0, _svgIcon.getLocalSVG)(name);
            if (url) {
                this.setData({
                    iconUrl: url,
                ...
                (0, _svgIcon.getIconifySVG)(name).then(function (res) {
                    that.setData({
                        iconUrl: res,
                        class: color ? 'icon' : 'default-icon'
                    });
...
exports.getLocalSVG = getLocalSVG;
exports.getIconifySVG = getIconifySVG;
exports.iconifySetup = iconifySetup;

var _mdiLight = __webpack_require__(2);

var _mdiLight2 = _interopRequireDefault(_mdiLight);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var ICONIFY_API = ''; // https://github.com/LarchLiu/weapp-svg-icon/blob/main/miniprogram/utils/svgIcon.ts
// svg-icon

var iconifyPresets = {
    MdiLight: _mdiLight2.default
};
var customerSvg = {
...
var iconifyPreload = ['carbon:portfolio'];
function svgNameSwtich(id) {
    return id.split(/:|-|_/).filter(Boolean).map(function (s) {
...
function presetsIconify(id) {
    ...
    var setData = iconifyPresets[prefix] ? iconifyPresets[prefix].icons : null;
    ...
    var svgAttributes = Object.assign({ xmlns: 'http://www.w3.org/2000/svg', 'xmlns:xlink': 'http://www.w3.org/1999/xlink' }, renderData.attributes);
    var svgAttributesStr = Object.keys(svgAttributes).map(function (attr) {
...
function getLocalSVG(id) {
    var name = svgNameSwtich(id);
    var data = customerSvg[name];
    if (data) {
        ...
        if (ICONIFY_API) {
            wx.request({
                ...
                success: function success(res) {
                    if (res.statusCode === 200) resolve(res.data);else reject(res.errMsg);
                },
...
function getIconifySVG(id) {
    return new Promise(function (resolve, reject) {
        var name = svgNameSwtich(id);
        iconifyApi(id).then(function (r) {
            var data = encodeSvg(r);
            wx.setStorage({
                key: name,
                data: data,
...
function iconifySetup(api) {
    if (api) {
        ICONIFY_API = api;
        for (var _iterator = iconifyPreload, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
            var _ref;
            ...
            if (!data) {
                getIconifySVG(id).catch(function (e) {
                    console.error(e);
                });
            }
        }
...
module.exports = require("@iconify-json/mdi-light");
